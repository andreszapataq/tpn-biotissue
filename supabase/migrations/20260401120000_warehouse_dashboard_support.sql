-- Mark Sointec Medical as warehouse
UPDATE public.institutions
SET is_warehouse = true
WHERE code = 'sointec-medical';

-- Drop and recreate with is_warehouse in return type
DROP FUNCTION IF EXISTS public.get_institutions_live_status();

CREATE FUNCTION public.get_institutions_live_status()
 RETURNS TABLE(institution_id uuid, institution_name text, institution_code text, is_warehouse boolean, active_patients bigint, active_procedures bigint, total_machines bigint, connected_machines bigint, available_machines bigint, maintenance_machines bigint, inactive_machines bigint, last_activity_at timestamp with time zone)
 LANGUAGE sql
 STABLE
 SET search_path TO 'public'
AS $function$
  with visible_institutions as (
    select i.id, i.name, i.code, i.is_warehouse
